...
public class DaoGames {
    ...
    public boolean create(BeanGames juegos, InputStream image){
        boolean flag = false;
        try{
            con = ConnectionMySQL.getConnection();
            cstm = con.prepareCall("{call sp_create(?,?,?)}");
            cstm.setString(1, juegos.getName());
            cstm.setBlob(2, imgGame);
            cstm.setString(3, juegos.getDatePremiere());
            cstm.execute();
            flag = true;
        }catch(SQLException e){
            System.out.println("Ocurrio un error: " + e.getMessage());
    ...
    public boolean update(BeanGames juegos){
        boolean flag = false;
        try{
            con = ConnectionMySQL.getConnection();
            cstm = con.prepareCall("{call sp_create(?,?,?,?,?)}");
            cstm.setInt(1, juegos.getIdGames());
            cstm.setString(2, juegos.getCategory_idCategory().getName());
            cstm.setString(3, juegos.getName());
            cstm.setString(4, juegos.getImgGame());
            cstm.setString(5, juegos.getDatePremiere());
